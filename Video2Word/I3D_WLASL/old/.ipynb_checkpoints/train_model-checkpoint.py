...
_VIDEO_DIR = 'train_processed'
_LABEL_MAP_PATH = 'labels.txt'

_CHECKPOINT_PATHS = {
  'rgb_imagenet': 'data/checkpointz/rgb_imagenet/model.ckpt',
  'flow_imagenet': 'data/checkpointz/flow_imagenet/model.ckpt',
  'training': 'data/checkpointz/training/model.ckpt'
}
...
#   learning_rate, rgb_input, flow_input, is_training, y = inputs
  learning_rate, rgb_input, is_training, y = inputs

  scores, loss, loss_minimize = outputs
#   rgb_saver, flow_saver, training_saver = savers
  rgb_saver, training_saver = savers
  ...
  def _check_acc(msg, dset, sess):
    ...
    for x_video, y_class in random.choices(dset, k=100):
      feed_dict = {
...
#           flow_input: flow_data(x_video, IMAGE_SIZE, nframes=NUM_FRAMES),
        ...
        if t % _CHECK_EVERY == 0:
            save_path = training_saver.save(sess, _CHECKPOINT_PATHS['training'])
            print('\nTraining model saved in path: %s' % save_path)
            rgb_save_path = rgb_saver.save(sess, _CHECKPOINT_PATHS['rgb_imagenet'])
            print('\nRGB model saved in path: %s' % rgb_save_path)
